"""07 — ResNet-50 Full Fine-tune: CV Convergence and Hold-out Results

ResNet-50 (ImageNet) full fine-tune backbone + masked mean-pool over up to 8 SVIs per building + small MLP with three heads (`building_type`, `bouwjaar`, `num_floors`).
...
Run:  uv run python scripts/figs_stage1_resnet.py
"""
import matplotlib
matplotlib.use("Agg")
...
RES_DIR = REPORTS_DIR / 'resnet50_ft'
fold_history   = [RES_DIR / f'pooled_fold{f}_history.json' for f in range(5)]
fold_val_preds = [RES_DIR / f'pooled_fold{f}_val_preds.parquet' for f in range(5)]
agg_path       = RES_DIR / 'pooled_aggregate_metrics.json'
ho_metrics_path = RES_DIR / 'holdout_metrics.json'
ho_preds_path   = RES_DIR / 'holdout_preds.parquet'
...
fig.tight_layout()
save_fig(fig, 'F1_resnet50_cv_val_curves', 'resnet')
...
fig.tight_layout()
save_fig(fig, 'F4_resnet50_loss_components', 'resnet')
...
save_table(cv_out, 'T1_resnet50_cv_per_fold')
...
print(f'best ckpt: models/stage1/pooled_resnet50_ft_fold{best_fold_idx}.pt')
cv_out
...
save_table(headline, 'T2_resnet50_holdout_headline')
headline.T
...
fig.tight_layout()
save_fig(fig, 'F2_resnet50_holdout_type_year', 'resnet')
...
fig.tight_layout()
save_fig(fig, 'F3_resnet50_per_class_per_city', 'resnet')
