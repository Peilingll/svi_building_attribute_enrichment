...
splits that every experiment reads. Raw inputs live in `data/raw/`; only the TABULA
workbook and the CBS polygons are tracked there, plus the four-city EP-Online slice
in `data/interim/`.
...
workbook `data/raw/tabula/tabula-values.xlsx`.
