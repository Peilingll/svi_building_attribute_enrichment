...
When the workbook contains several subperiod assemblies within one TABULA-NL period, the implementation selects the assembly with the highest U-value.
Window U-values are assigned directly by construction period.

The generated values matched the WebTool values for the wall, roof, and floor of the NL.01 terraced house and for the wall and roof of the NL.01 apartment block.
For the NL.01 apartment block floor, the WebTool reports 1.14\,$\mathrm{W/(m^2\,K)}$, but this value is not present in the existing state construction library used by the generator.
The implementation therefore uses 1.7241\,$\mathrm{W/(m^2\,K)}$ from construction \texttt{NL.Floor.ReEx.01.02}, which provides a traceable workbook value for the selected cavity wall variant.
